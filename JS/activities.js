...
function mostrarActividadesNuevas(datos) {
    ...
    const actividadesNuevas = datos.filter(
    actividad => revisarFecha(actividad.date)
    ...
    actividadesNuevas.forEach(element => {
        ...
        if (element.link != "-") {
            document.getElementById("nextActivitiesCarrousel").innerHTML += `
                ...
                <a href="${element.link}" target="_blank" rel="noopener noreferrer" class="buttonB">Ver más</a>
            </div>
        `;
        } else {
            document.getElementById("nextActivitiesCarrousel").innerHTML += `
...
async function iniciarCarga() {
    const actividades = await obtenerDatosDeActividadesPasadas('https://edward5126.github.io/RAS-FIUADY/JSON/Activities.json');
    ...
    const actividadesNuevas = await obtenerDatosDeActividadesSiguientes('https://edward5126.github.io/RAS-FIUADY/JSON/nextActivities.json');
